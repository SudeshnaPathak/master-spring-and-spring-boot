...
## Deployment
1. `npm run build` to generate optimized production ready build files.
2. Go to AWS Console -> S3
3. Create an S3 Bucket
4. Configure unique bucket name
5. Disable: Block all public access 
6. Upload all the build files to the S3 bucket, remember to upload the static folder separately using add folder
7. Go to Properties -> Static Website Hosting -> Edit
8. Enable Static hosting 
7. Index Document: index.html
9. Still we will not be able to access through the public URL
10. Now go to Permissions -> Bucket Policies -> Edit
...
11. Add this to the bucket policy.
12. Access the static website through the public URL available in static website section.
